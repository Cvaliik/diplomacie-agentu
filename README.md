...
Tahy plánuje workflow `rozvrh` (`.github/workflows/rozvrh.yml`). Běží každých 30 minut a sám si spočítá, kolik tahů už mělo být
odehráno: 3 za každý celý den od `meta.day1_date` plus dnešní sloty 07:00, 13:00 a 20:00 pražského času. Rozvrh se tím
opravuje sám: výpadek se dožene do 30 minut a chybí-li dva tahy, odehrají se dva po sobě. Když je co hrát, rozvrh spustí
workflow `tah` (`.github/workflows/tah.yml`), který tah odehraje, commitne a po slotu 3 napíše kroniku. Po slotu 3 vznikne kronika dne. Klíč je v nastavení repa jako secret `ARDAN_API_KEY`. Číslo tahu se vždy bere ze `state.json`.
...
- **Dohrání selhaného slotu:** selhaný běh je červený a nic necommitne, `state.json` zůstává na posledním dobrém tahu.
  Po opravě příčiny spusť v GitHubu *Actions → tah → Run workflow* s `mode` = `turn` (hraje bez ohledu na rozvrh). Odehraje se tah, který chybí;
...
- **Suchý běh:** *Run workflow* s `mode` = `dry-model` sestaví prompty bez volání API a uloží je jako artefakt
  `debug-prompty`.
- **Prázdné kontroly** běží jen ve workflow `rozvrh` a končí za pár sekund, bez závislostí a bez klíče. Skutečné tahy
  a kroniky jsou jen ve workflow `tah`: v Actions stačí vlevo vybrat *tah*. Běhy spuštěné rozvrhem se jmenují "tah",
  ruční "ručně turn" a "ručně dry-model"; shrnutí běhu ukazuje "tah NNN (den D, slot S)" a commit se jmenuje stejně.
